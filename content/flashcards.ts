...
export const decks = [
  ...
  { id: 'networking', name: 'Networking & Infrastructure', description: 'Load balancing, CDN, proxies, and more' },
  { id: 'case-studies', name: 'System Design Case Studies', description: 'Classic interview problems' },
  { id: 'interview-method', name: 'Interview Methodology', description: 'Framework and techniques for system design interviews' },
]
...
export const flashcards: Flashcard[] = [
  ...
  // NETWORKING & INFRASTRUCTURE DECK
  {
    id: 'net-1',
    deck: 'networking',
    front: 'What is a load balancer?',
    back: 'Distributes incoming traffic across multiple servers. Enables horizontal scaling, improves availability, performs health checks. Algorithms: round robin, least connections, IP hash. Types: L4 (transport) and L7 (application).',
    category: 'networking',
  },
  {
    id: 'net-2',
    deck: 'networking',
    front: 'Round Robin vs. Least Connections load balancing?',
    back: 'Round Robin: sequential distribution (1→2→3→1). Simple but ignores server load. Least Connections: routes to server with fewest active connections. Better for varying request durations and long-lived connections.',
    category: 'networking',
  },
  {
    id: 'net-3',
    ...
    back: 'L4: transport layer (TCP/UDP). Faster, no content inspection. Routes by IP/port. L7: application layer (HTTP). Can route by URL path, headers, cookies. Can terminate SSL. More flexible but slower.',
    category: 'networking',
  },
  {
    id: 'net-4',
    deck: 'networking',
    front: 'What is a reverse proxy?',
    back: 'Sits in front of servers, receiving requests from internet. Clients don\'t know backend servers. Use cases: load balancing, SSL termination, caching, compression, security (WAF). Examples: nginx, HAProxy, Cloudflare.',
    category: 'networking',
  },
  {
    id: 'net-5',
    deck: 'networking',
    front: 'Forward Proxy vs. Reverse Proxy?',
    back: 'Forward: sits in front of clients, forwards to internet (content filtering, anonymity). Reverse: sits in front of servers, receives from internet (load balancing, caching, security). Forward hides client. Reverse hides server.',
    category: 'networking',
  },
  {
    id: 'net-6',
    ...
    back: 'Distributes keys across nodes using a hash ring. When node added/removed, only 1/N keys remap (vs ~90% with mod hashing). Used in: distributed caches, database sharding, CDNs. Virtual nodes improve distribution.',
    category: 'networking',
  },
  {
    id: 'net-7',
    deck: 'networking',
    front: 'Why use virtual nodes in consistent hashing?',
    back: 'Each physical server has multiple positions on the ring. Prevents uneven distribution when servers hash to clustered positions. Allows weighted nodes (more capacity = more virtual nodes). Smoother rebalancing.',
    category: 'networking',
  },
  {
    id: 'net-8',
    ...
    back: 'Consistency, Availability, Partition Tolerance. In a distributed system, you can only guarantee two. Networks fail, so P is required. Choose: CP (reject requests during partition) or AP (serve potentially stale data).',
    category: 'networking',
  },
  {
    id: 'net-9',
    deck: 'networking',
    front: 'CP vs. AP databases?',
    back: 'CP (Consistency + Partition tolerance): MongoDB, HBase, Zookeeper. Reject requests during partition. AP (Availability + Partition tolerance): Cassandra, DynamoDB, CouchDB. Serve requests, resolve conflicts later.',
    category: 'networking',
  },
  {
    id: 'net-10',
    deck: 'networking',
    front: 'What is a CDN?',
    back: 'Content Delivery Network: geographically distributed servers that cache content close to users. Reduces latency (closer server) and origin load (caching). Push (upload) vs Pull (fetch on miss). Examples: CloudFront, Cloudflare.',
    category: 'networking',
  },
  {
    id: 'net-11',
    deck: 'networking',
    front: 'CDN cache invalidation strategies?',
    back: 'Purge: delete specific URLs. TTL: automatic expiry. Versioned URLs: app.v2.js. Cache-Control headers. ETags for conditional requests. Trade-off: freshness vs. cache hit ratio.',
    category: 'networking',
  },
  {
    id: 'net-12',
    deck: 'networking',
    front: 'Active-Passive vs. Active-Active redundancy?',
    back: 'Active-Passive: primary handles traffic, standby takes over on failure. Simpler but standby is idle. Active-Active: all instances handle traffic simultaneously. Better utilization but needs state sync.',
    category: 'networking',
  },
  {
    id: 'net-13',
    deck: 'networking',
    front: 'What is SPOF and how to eliminate it?',
    back: 'Single Point of Failure: component whose failure causes system failure. Eliminate by: redundant servers, multiple load balancers, database replication, multi-region deployment, multiple network paths.',
    category: 'networking',
  },
  {
    id: 'net-14',
    deck: 'networking',
    front: 'Synchronous vs. Asynchronous replication?',
    back: 'Synchronous: wait for replica confirmation before acknowledging write. Consistent but slower. Asynchronous: acknowledge immediately, replicate in background. Faster but may lose recent writes on failure.',
    category: 'networking',
  },
  {
    id: 'net-15',
    deck: 'networking',
    front: 'WebSocket vs. HTTP Polling vs. SSE?',
    back: 'HTTP Polling: repeated requests (inefficient). Long Polling: hold connection until data. WebSocket: persistent bidirectional (chat, gaming). SSE: server-to-client only (notifications). Choose based on use case.',
    category: 'networking',
  },
  {
    id: 'net-16',
    ...
    back: 'Decrypting HTTPS at load balancer/proxy, forwarding HTTP to backend. Benefits: offload CPU from app servers, centralized certificate management. Requires: secure internal network (or re-encrypt).',
    category: 'networking',
  },
  {
    id: 'net-17',
    deck: 'networking',
    front: 'What is geohashing?',
    back: 'Encoding latitude/longitude into a string. Nearby locations share prefix. Enables efficient range queries (find all in area by prefix). Used in: Uber driver matching, local search, CDN routing.',
    category: 'networking',
  },
  {
    id: 'net-18',
    deck: 'networking',
    front: 'RPO vs. RTO?',
    back: 'RPO (Recovery Point Objective): maximum acceptable data loss (time). RTO (Recovery Time Objective): maximum acceptable downtime. RPO determines backup frequency. RTO determines failover speed.',
    category: 'networking',
  },
  ...
  // CASE STUDIES DECK
  {
    id: 'cs-1',
    deck: 'case-studies',
    front: 'URL Shortener: How to generate short URLs?',
    back: 'Base62 encoding (a-z, A-Z, 0-9). Options: 1) Hash URL + take first N chars (collision risk). 2) Auto-increment ID + base62 (predictable). 3) Distributed ID generator (Snowflake). 7 chars = 3.5 trillion URLs.',
    category: 'system-design',
  },
  {
    id: 'cs-2',
    deck: 'case-studies',
    front: 'URL Shortener: 301 vs. 302 redirect?',
    back: '301 (Permanent): browser caches, fewer server hits, lose analytics. 302 (Temporary): always hits server, full analytics. Use 302 if tracking clicks is important.',
    category: 'system-design',
  },
  {
    id: 'cs-3',
    deck: 'case-studies',
    front: 'Twitter: What is the fan-out problem?',
    back: 'When celebrity with millions of followers tweets, fan-out on write means millions of writes. Solution: hybrid approach - fan-out on write for regular users, fan-out on read for celebrities (>10K followers).',
    category: 'system-design',
  },
  {
    id: 'cs-4',
    deck: 'case-studies',
    front: 'Twitter: Fan-out on Write vs. Read?',
    back: 'Write: push tweet to followers\' timelines (fast reads, slow writes). Read: fetch from followed users at read time (slow reads, fast writes). Hybrid: write for most, read for celebrities.',
    category: 'system-design',
  },
  {
    id: 'cs-5',
    deck: 'case-studies',
    front: 'Instagram: How to handle photo uploads?',
    back: 'Direct upload to S3 with pre-signed URLs (bypass app servers). Async processing: accept upload, queue for resize/filter, notify when ready. Store multiple sizes (thumbnail, small, medium, large).',
    category: 'system-design',
  },
  {
    id: 'cs-6',
    deck: 'case-studies',
    front: 'Instagram: How to implement Stories?',
    back: 'Ephemeral content with 24-hour TTL. Store in Redis with automatic expiry. Aggregate stories from followed users on read. Separate from permanent posts for different access patterns.',
    category: 'system-design',
  },
  {
    id: 'cs-7',
    deck: 'case-studies',
    front: 'Uber: How to find nearby drivers?',
    back: 'Geohashing or quadtrees for location indexing. Geohash: divide world into grid cells, nearby locations share prefix. Store in Redis with GEOADD/GEORADIUS. Query by prefix for fast lookups.',
    category: 'system-design',
  },
  {
    id: 'cs-8',
    deck: 'case-studies',
    front: 'Uber: Real-time location tracking?',
    back: 'Drivers send location every 4 seconds via WebSocket. Store in Redis (in-memory, fast). Update rider app via WebSocket. Use Redis pub/sub for scaling across servers.',
    category: 'system-design',
  },
  {
    id: 'cs-9',
    deck: 'case-studies',
    front: 'YouTube: What is adaptive bitrate streaming?',
    back: 'Video split into segments (2-10 sec). Available at multiple quality levels. Client downloads manifest listing options. Client switches quality based on bandwidth. Protocols: HLS, DASH.',
    category: 'system-design',
  },
  {
    id: 'cs-10',
    deck: 'case-studies',
    front: 'YouTube: Video transcoding pipeline?',
    back: 'Convert uploaded videos to multiple formats (360p-4K) and codecs (H.264, VP9). Use distributed workers (FFmpeg). Queue jobs for processing. Store segments in object storage. CDN caches popular content.',
    category: 'system-design',
  },
  {
    id: 'cs-11',
    deck: 'case-studies',
    front: 'YouTube: How to handle view counts?',
    back: 'Eventual consistency acceptable. Log view events to Kafka. Aggregate with Flink/Spark in real-time. Batch reconciliation for accuracy. Avoid write contention on single counter.',
    category: 'system-design',
  },
  {
    id: 'cs-12',
    deck: 'case-studies',
    front: 'Pastebin: Key design considerations?',
    back: 'Similar to URL shortener but store content. Random key generation. Content-addressable storage (hash content). Support expiration. Rate limiting. Syntax highlighting (client-side).',
    category: 'system-design',
  },
  {
    id: 'cs-13',
    deck: 'case-studies',
    front: 'Dropbox: How to handle large file sync?',
    back: 'Chunk files into blocks (4MB). Hash each block. Only upload changed blocks (deduplication). Sync metadata first, then blocks. Resume interrupted uploads. Conflict resolution for concurrent edits.',
    category: 'system-design',
  },
  {
    id: 'cs-14',
    deck: 'case-studies',
    front: 'Web Crawler: Key challenges?',
    back: 'Politeness (respect robots.txt, rate limits). Deduplication (avoid recrawling same page). Priority (important pages first). Distributed coordination. Handle dynamic content (JavaScript rendering).',
    category: 'system-design',
  },
  {
    id: 'cs-15',
    deck: 'case-studies',
    front: 'Ticketmaster: How to prevent overselling?',
    back: 'Distributed locking on seats during purchase. Short reservation timeout (10 min). Optimistic locking with version numbers. Queue for high-demand events. Two-phase commit for payment + reservation.',
    category: 'system-design',
  },

  // INTERVIEW METHODOLOGY DECK
  {
    id: 'im-1',
    deck: 'interview-method',
    front: 'What are the 5 steps of the system design interview framework?',
    back: '1. Understand the Problem (5-7 min): requirements. 2. Define Entities & API (3-5 min). 3. High-Level Design (10-15 min): basic working system. 4. Deep Dives (15-20 min): address non-functional requirements. 5. Final Design & Wrap-up (5 min).',
  ...
  },
  {
    ...
    back: 'Functional: what the system does (features, user stories). Example: "Users can shorten URLs". Non-functional: how the system operates (quality attributes). Example: "99.99% availability, <100ms latency, scale to 1B URLs".',
    category: 'methodology',
  },
  {
    id: 'im-3',
    deck: 'interview-method',
    front: 'What does "below the line" mean in system design?',
    back: 'Features considered out of scope for the design. Mention them to show awareness, but don\'t design them. Example: authentication, analytics, admin features. Confirm with interviewer if unsure.',
    category: 'methodology',
  },
  {
    id: 'im-4',
    deck: 'interview-method',
    front: 'What is read-to-write ratio and why does it matter?',
    back: 'Ratio of read operations to write operations. URL shortener: 1000:1 (read-heavy) → cache aggressively. Chat: 1:1 → optimize both. Determines: database choice, caching strategy, scaling approach.',
    category: 'methodology',
  },
  {
    id: 'im-5',
    deck: 'interview-method',
    front: 'How should you present solutions in deep dives?',
    back: 'Present multiple options: Bad → Good → Great. For each: explain how it works, advantages, disadvantages. Make a clear recommendation with rationale. Show trade-off thinking.',
    category: 'methodology',
  },
  {
    id: 'im-6',
    deck: 'interview-method',
    front: 'What key numbers should you know for back-of-envelope calculations?',
    back: '1 day ≈ 100K seconds. 1 year ≈ 30M seconds. Redis: 100K QPS. MySQL: 10K QPS (reads). Network same datacenter: 0.5ms. Cross-region: 50ms. SSD: 0.1ms. HDD: 10ms.',
    category: 'methodology',
  },
  {
    id: 'im-7',
    deck: 'interview-method',
    front: 'How to estimate storage requirements?',
    back: 'Formula: bytes per record × number of records. Round generously (500B → 1KB). Example: URL shortener: 500 bytes × 1B URLs = 500GB. Single SSD handles this.',
    category: 'methodology',
  },
  {
    id: 'im-8',
    deck: 'interview-method',
    front: 'How to estimate QPS (queries per second)?',
    back: 'Formula: daily requests ÷ 86,400 (or ÷ 100K for easy math). Peak = 3-5× average. Example: 1B requests/day ÷ 100K = 10K QPS average, 50K peak.',
    category: 'methodology',
  },
  {
    id: 'im-9',
    deck: 'interview-method',
    front: 'What is the 80/20 rule for caching?',
    back: '20% of data serves 80% of traffic. Cache the hot 20% to handle most requests. Example: 1B URLs × 20% × 500 bytes = 100GB cache handles 80% of traffic.',
    category: 'methodology',
  },
  {
    id: 'im-10',
    deck: 'interview-method',
    front: 'How to approach scalability in deep dives?',
    back: 'Identify the bottleneck first. Read scaling: cache, read replicas, CDN. Write scaling: sharding, queues. Compute: horizontal scaling + load balancer. Storage: blob storage, partitioning.',
    category: 'methodology',
  },
  {
    id: 'im-11',
    deck: 'interview-method',
    front: 'What failure modes should you consider?',
    back: 'For each critical component ask: "What if this fails?" Database: replication, backups. Cache: fail-open vs fail-closed. Service: health checks, circuit breakers. Network: retries, timeouts, idempotency.',
    category: 'methodology',
  },
  {
    id: 'im-12',
    deck: 'interview-method',
    front: 'When to use which database type?',
    back: 'SQL: complex relationships, ACID needed, structured data. NoSQL: massive scale, flexible schema, simple access patterns. Key-value: caching, sessions. Document: hierarchical data. Wide-column: time-series. Graph: relationships.',
    category: 'methodology',
  },
  {
    id: 'im-13',
    deck: 'interview-method',
    front: 'What REST HTTP methods to use in API design?',
    back: 'POST: create new resource. GET: read resource. PUT: update entire resource. PATCH: partial update. DELETE: remove resource. Use proper status codes: 200 (OK), 201 (created), 400 (bad request), 404 (not found), 429 (rate limited).',
    category: 'methodology',
  },
  {
    id: 'im-14',
    deck: 'interview-method',
    front: 'How to handle unique ID generation at scale?',
    back: 'Options: 1) Auto-increment (single DB, not scalable). 2) UUID (random, long). 3) Snowflake (timestamp + machine + sequence). 4) Counter with base62 encoding. For URL shortener: counter + base62 gives short, unique codes.',
    category: 'methodology',
  },
  {
    id: 'im-15',
    deck: 'interview-method',
    front: 'What common scaling patterns should you know?',
    back: 'Caching layer (Redis). Load balancing. Database sharding. Read replicas. Message queues (Kafka). CDN for static content. Microservices for independent scaling. Async processing for long operations.',
    category: 'methodology',
  },
]
