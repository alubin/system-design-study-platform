...
export const categories = [
  ...
  { id: 'networking', name: 'Networking & Infrastructure' },
    ...
    concepts: [
      {
        ...
        content: 'Distributing incoming network traffic across multiple servers to ensure no single server becomes overwhelmed.',
        eli5: 'Like having multiple checkout lanes at a grocery store - customers are directed to the lane with the shortest queue so everyone gets served faster.',
        technical: 'Load balancers sit between clients and servers, routing requests based on algorithms. They enable horizontal scaling, improve availability (if one server fails, traffic routes to others), and can perform health checks to remove unhealthy servers from rotation.',
      },
      {
        title: 'Round Robin',
        content: 'Simplest algorithm - requests are distributed sequentially across servers.',
        eli5: 'Like dealing cards to players - each person gets one card in turn, then repeat.',
        technical: 'Server 1 gets request 1, Server 2 gets request 2, etc. Simple but doesn\'t account for server capacity or current load. Weighted round robin assigns more requests to more powerful servers.',
      },
      {
        title: 'Least Connections',
        content: 'Routes requests to the server with fewest active connections.',
        eli5: 'Like choosing the shortest line at the bank - you go where the fewest people are waiting.',
        technical: 'Better for long-lived connections (WebSockets, database connections). Tracks active connections per server. Weighted least connections considers server capacity.',
      },
      {
        title: 'IP Hash / Session Affinity',
        content: 'Routes requests from the same client to the same server consistently.',
        eli5: 'Like always going to the same barber who knows your haircut preference.',
        technical: 'Hash of client IP determines server. Ensures session data stays on same server (sticky sessions). Problem: uneven distribution if some IPs generate more traffic. Better to externalize session state.',
      },
      {
        title: 'Layer 4 vs Layer 7 Load Balancing',
        content: 'L4 operates at transport layer (TCP/UDP), L7 at application layer (HTTP).',
        eli5: 'L4 is like routing mail by zip code only. L7 is like reading the letter and deciding where to send it based on content.',
        technical: 'L4: faster, based on IP/port, no request inspection. L7: can route based on URL path, headers, cookies. L7 can do content-based routing (/api to API servers, /static to CDN). L7 can terminate SSL.',
      },
    ],
    terminology: [
      { term: 'Health Check', definition: 'Periodic requests to verify server is responding correctly' },
      { term: 'Sticky Session', definition: 'Routing same client to same server for session continuity' },
      { term: 'Failover', definition: 'Automatic switch to backup when primary fails' },
      { term: 'Active-Passive', definition: 'One active load balancer with standby backup' },
      { term: 'Active-Active', definition: 'Multiple load balancers sharing traffic' },
    ],
    commonMistakes: [
      'Using sticky sessions instead of externalizing session state',
      'Not implementing health checks - routing to dead servers',
      'Single load balancer becoming single point of failure',
      'Not considering SSL termination overhead on L7 balancers',
      'Choosing wrong algorithm for workload (round robin for varied response times)',
    ],
    relatedTopics: ['proxies', 'cdn', 'caching'],
  },
  {
    id: 'proxies',
    title: 'Forward & Reverse Proxies',
    category: 'networking',
    concepts: [
      {
        title: 'What is a Proxy?',
        content: 'An intermediary server that sits between clients and servers, forwarding requests and responses.',
        eli5: 'Like a personal assistant who makes calls on your behalf - the person on the other end talks to your assistant, not directly to you.',
        technical: 'Proxies can filter, log, transform, or cache requests. They provide abstraction between client and server, enabling security, performance, and management capabilities.',
      },
      {
        title: 'Forward Proxy',
        content: 'Sits in front of clients, forwarding their requests to the internet.',
        eli5: 'Like a secretary who makes phone calls for the whole office - everyone\'s calls go through them.',
        technical: 'Client configures browser to use proxy. Use cases: content filtering (block sites), anonymity (hide client IP), caching (shared cache for office), bypassing geo-restrictions. Examples: Squid, corporate proxies.',
      },
      {
        title: 'Reverse Proxy',
        content: 'Sits in front of servers, receiving requests from the internet.',
        eli5: 'Like a receptionist at a company - all visitors talk to the receptionist first, who then directs them to the right person.',
        technical: 'Clients don\'t know about backend servers. Use cases: load balancing, SSL termination, caching, compression, security (WAF), A/B testing, canary deployments. Examples: nginx, HAProxy, Cloudflare.',
      },
      {
        title: 'Common Reverse Proxy Features',
        content: 'SSL termination, compression, caching, request routing.',
        eli5: 'The receptionist can also translate languages (SSL), summarize messages (compression), and remember frequent answers (caching).',
        technical: 'SSL termination: handle HTTPS at proxy, HTTP to backend (faster, simpler certs). Compression: gzip responses. Static file caching: serve from proxy, don\'t hit backend. Path-based routing: /api to app servers, /images to CDN.',
      },
    ],
    terminology: [
      { term: 'SSL Termination', definition: 'Decrypting HTTPS at proxy before forwarding as HTTP' },
      { term: 'Origin Server', definition: 'The actual backend server behind a proxy' },
      { term: 'Edge Server', definition: 'Proxy server close to end users (CDN edge)' },
      { term: 'WAF', definition: 'Web Application Firewall - filters malicious requests' },
    ],
    commonMistakes: [
      'Exposing origin servers directly, bypassing proxy protections',
      'Not preserving client IP in X-Forwarded-For header',
      'SSL termination without secure internal network',
      'Caching private/authenticated content accidentally',
      'Not setting proper cache headers for proxy caching',
    ],
    relatedTopics: ['load-balancing', 'cdn', 'caching'],
  },
  {
    ...
    concepts: [
      {
        ...
        content: 'When using hash(key) % N servers, adding/removing a server remaps almost all keys.',
        eli5: 'If you organize books by "author name mod 5 shelves" and add a 6th shelf, almost every book moves.',
        technical: 'hash(key) % N means adding one server changes N to N+1, causing ~(N-1)/N keys to remap. With 10 servers, adding 1 remaps ~90% of keys. Catastrophic for caches.',
      },
      {
        title: 'Consistent Hashing Solution',
        content: 'Arrange servers and keys on a ring. Each key goes to the next server clockwise.',
        eli5: 'Imagine servers are towns on a circular road. Each package goes to the nearest town clockwise. Adding a town only affects packages between it and the previous town.',
        technical: 'Hash servers and keys to positions on a ring (0 to 2^32). Key routes to first server found clockwise. Adding/removing server only affects keys between it and predecessor. Only 1/N keys remap on average.',
      },
      {
        title: 'Virtual Nodes',
        content: 'Each physical server has multiple positions on the ring for better distribution.',
        eli5: 'Each town has multiple mailboxes around the road instead of just one, so packages are distributed more evenly.',
        technical: 'Without virtual nodes, uneven distribution if servers hash to clustered positions. Virtual nodes: Server A appears as A1, A2, A3... at different ring positions. More uniform load. Easier rebalancing when nodes differ in capacity.',
      },
      {
        title: 'Use Cases',
        content: 'Distributed caches, database sharding, load balancing, CDNs.',
        eli5: 'Any time you need to spread work across machines and want adding/removing machines to cause minimal disruption.',
        technical: 'Memcached, Redis Cluster, DynamoDB, Cassandra, Akamai CDN all use consistent hashing. Essential for any distributed system that needs stable key-to-node mapping with dynamic membership.',
      },
    ],
    terminology: [
      { term: 'Hash Ring', definition: 'Circular arrangement of hash space from 0 to max' },
      { term: 'Virtual Node', definition: 'Multiple ring positions for one physical server' },
      { term: 'Rebalancing', definition: 'Redistributing data when nodes join/leave' },
      { term: 'Partition', definition: 'Portion of ring owned by a node' },
    ],
    commonMistakes: [
      'Too few virtual nodes - uneven distribution',
      'Not accounting for node capacity differences',
      'Ignoring hot keys that bypass distribution',
      'Using simple mod hashing for distributed systems',
      'Not planning for data migration during rebalancing',
    ],
    relatedTopics: ['caching', 'database-selection', 'data-modeling'],
  },
  {
    id: 'cap-theorem',
    title: 'CAP Theorem',
    ...
    concepts: [
      {
        ...
        content: 'In a distributed system, you can only guarantee two of: Consistency, Availability, Partition Tolerance.',
        eli5: 'Imagine three friends who always want to agree (Consistency), always answer calls (Availability), and can handle phone lines being down (Partition Tolerance). When lines are down, they can\'t both agree AND always answer.',
        technical: 'Consistency: all nodes see same data. Availability: every request gets a response. Partition Tolerance: system works despite network failures. In practice, P is required (networks fail), so you choose between C and A.',
      },
      {
        title: 'Consistency (C)',
        content: 'All nodes see the same data at the same time.',
        eli5: 'Everyone in the group has the same information instantly - no one is out of date.',
        technical: 'Linearizability: reads return most recent write. Requires coordination between nodes. Example: traditional RDBMS with strong consistency. Trade-off: slower operations, lower availability during failures.',
      },
      {
        title: 'Availability (A)',
        content: 'Every request receives a response (success or failure).',
        eli5: 'The store is always open - you\'ll always get an answer, though the item might be slightly outdated.',
        technical: 'System remains operational and responsive. No timeouts or errors due to system state. May return stale data during partitions. Example: eventually consistent NoSQL databases.',
      },
      {
        title: 'Partition Tolerance (P)',
        content: 'System continues operating despite network partitions between nodes.',
        eli5: 'Even if phone lines between offices are down, each office keeps working.',
        technical: 'Network can drop or delay messages. Nodes become unreachable. Required in distributed systems - networks are unreliable. The question is: during partition, do you prefer C (reject requests) or A (serve potentially stale data)?',
      },
      {
        title: 'CP vs AP Systems',
        content: 'Choosing between consistency and availability when partitions occur.',
        eli5: 'When phones are down: CP = wait until everyone agrees before announcing. AP = each office announces their best guess.',
        technical: 'CP systems (MongoDB, HBase, Zookeeper): reject requests during partition to maintain consistency. AP systems (Cassandra, CouchDB, DynamoDB): serve requests during partition, resolve conflicts later. Most systems are tunable between CP and AP.',
      },
    ],
    terminology: [
      { term: 'Network Partition', definition: 'Communication failure between nodes' },
      { term: 'Split Brain', definition: 'Partition where both sides think they are primary' },
      { term: 'Quorum', definition: 'Minimum nodes that must agree (usually majority)' },
      { term: 'Eventual Consistency', definition: 'System converges to consistent state over time' },
    ],
    commonMistakes: [
      'Thinking you can have all three (C, A, P) simultaneously',
      'Ignoring partition tolerance - networks always fail eventually',
      'Not understanding that CAP applies during failures, not normal operation',
      'Choosing CP when AP is acceptable (over-engineering consistency)',
      'Not planning conflict resolution for AP systems',
    ],
    relatedTopics: ['database-selection', 'distributed-locking', 'event-driven'],
  },
  {
    id: 'cdn',
    ...
    concepts: [
      {
        title: 'What is a CDN?',
        content: 'Geographically distributed network of servers that cache content closer to users.',
        eli5: 'Instead of everyone ordering from one pizza shop downtown, there are pizza shops in every neighborhood with the same menu.',
        technical: 'CDN has edge servers worldwide. When user requests content, nearest edge serves it. If not cached, edge fetches from origin, caches, and serves. Reduces latency (closer server) and origin load (caching).',
      },
      {
        title: 'Static vs Dynamic Content',
        content: 'CDNs excel at static content, can also help with dynamic content.',
        eli5: 'Static = same menu for everyone (easy to copy to each shop). Dynamic = custom orders (harder, but shop can still be closer).',
        technical: 'Static (images, CSS, JS): cache at edge, long TTL, high hit rate. Dynamic (API responses): cache briefly or not at all, but edge closer to user still reduces latency. Modern CDNs run code at edge (Cloudflare Workers).',
      },
      {
        title: 'Push vs Pull CDN',
        content: 'Push: you upload content to CDN. Pull: CDN fetches from origin on first request.',
        eli5: 'Push = you deliver pizzas to each shop in advance. Pull = shops order from you when a customer asks.',
        technical: 'Push (origin push): good for large files, predictable content, you control cache timing. Pull (origin pull): simpler, CDN fetches on cache miss, good for dynamic sites. Most CDNs use pull with configurable TTL.',
      },
      {
        title: 'Cache Invalidation',
        content: 'Removing stale content from CDN cache before TTL expires.',
        eli5: 'Telling all pizza shops to throw out yesterday\'s specials because the menu changed.',
        technical: 'Purge: delete specific URLs from cache. Invalidate: mark as stale (re-validate with origin). Versioned URLs: append version to filename (app.v2.js). Headers: Cache-Control, ETags for conditional requests.',
      },
      {
        title: 'CDN Security Features',
        content: 'DDoS protection, WAF, SSL/TLS termination.',
        eli5: 'The neighborhood pizza shops also have security guards and check IDs.',
        technical: 'DDoS mitigation: absorb attack traffic across distributed network. WAF: filter malicious requests (SQL injection, XSS). SSL at edge: HTTPS without origin certificates. Bot protection, rate limiting.',
      },
    ],
    terminology: [
      { term: 'Edge Server', definition: 'CDN server close to end users' },
      { term: 'Origin Server', definition: 'Your actual server that CDN fetches from' },
      { term: 'PoP', definition: 'Point of Presence - CDN data center location' },
      { term: 'TTL', definition: 'Time To Live - how long to cache before re-fetching' },
      { term: 'Cache Hit Ratio', definition: 'Percentage of requests served from cache' },
    ],
    commonMistakes: [
      'Caching private/authenticated content (data leak)',
      'Not versioning static assets (stale cache after deploy)',
      'TTL too short (low hit rate) or too long (stale content)',
      'Not using CDN for static assets (unnecessary origin load)',
      'Ignoring cache headers from origin server',
    ],
    relatedTopics: ['caching', 'proxies', 'load-balancing'],
  },
  {
    id: 'redundancy',
    ...
    concepts: [
      {
        title: 'Why Redundancy?',
        content: 'Duplicate critical components so system continues working when components fail.',
        eli5: 'Like having a spare tire in your car - when one goes flat, you can keep driving.',
        technical: 'Single points of failure (SPOF) cause system outages. Redundancy eliminates SPOF. Apply at all layers: servers, databases, load balancers, data centers, network paths. Trade-off: cost vs. availability.',
      },
      {
        title: 'Active-Passive (Failover)',
        content: 'Primary handles traffic, standby takes over on failure.',
        eli5: 'One pilot flies while the co-pilot is ready to take over if needed.',
        technical: 'Heartbeat monitor detects primary failure. Standby promoted to primary. Simpler but standby is idle (wasted resources). Recovery time: seconds to minutes. Used for databases, load balancers.',
      },
      {
        title: 'Active-Active',
        content: 'Multiple instances handle traffic simultaneously.',
        eli5: 'Multiple checkout lanes all open at once - if one closes, others absorb the traffic.',
        technical: 'All instances process requests. Load balanced across them. Better resource utilization. More complex: need state synchronization, conflict resolution. Used for stateless services, CDN edge servers.',
      },
      {
        title: 'Database Replication',
        content: 'Copying data across multiple database instances.',
        eli5: 'Having copies of important documents in multiple safes in different locations.',
        technical: 'Primary-Replica (Master-Slave): primary handles writes, replicas handle reads. Synchronous replication: wait for replica confirmation (consistent, slower). Asynchronous: don\'t wait (faster, may lose recent writes on failure).',
      },
      {
        title: 'Geographic Redundancy',
        content: 'Distributing systems across multiple data centers/regions.',
        eli5: 'Having offices in different cities so an earthquake in one doesn\'t shut down the company.',
        technical: 'Protects against: data center outages, natural disasters, regional network issues. Challenges: data synchronization, latency, consistency. Active-active across regions or active-passive with failover.',
      },
    ],
    terminology: [
      { term: 'SPOF', definition: 'Single Point of Failure - component whose failure causes system failure' },
      { term: 'Failover', definition: 'Switching to redundant component on failure' },
      { term: 'Hot Standby', definition: 'Backup ready to take over immediately' },
      { term: 'Cold Standby', definition: 'Backup that needs time to start up' },
      { term: 'RPO', definition: 'Recovery Point Objective - acceptable data loss (time)' },
      { term: 'RTO', definition: 'Recovery Time Objective - acceptable downtime' },
    ],
    commonMistakes: [
      'Single load balancer in front of redundant servers',
      'Synchronous replication across regions (too slow)',
      'Not testing failover procedures regularly',
      'Assuming cloud provider handles all redundancy',
      'Not considering data consistency during failover',
    ],
    relatedTopics: ['load-balancing', 'database-selection', 'cap-theorem'],
  },
  {
    ...
    concepts: [
      {
        ...
        content: 'Client sends request, waits for response. Connection closes after response.',
        eli5: 'Like sending a letter and waiting for a reply - one question, one answer.',
        technical: 'Stateless, simple, widely supported. HTTP/1.1: new connection per request (or keep-alive). HTTP/2: multiplexed streams. Good for: CRUD operations, APIs. Limitation: server can\'t push data to client.',
      },
      {
        title: 'Short Polling',
        content: 'Client repeatedly requests updates at regular intervals.',
        eli5: 'Checking your mailbox every 5 minutes to see if there\'s new mail.',
        technical: 'setInterval to fetch updates. Simple to implement. Inefficient: many requests with no new data. Latency: updates delayed until next poll. Use when: updates are rare, simplicity matters, real-time not critical.',
      },
      {
        title: 'Long Polling',
        content: 'Server holds request open until new data is available or timeout.',
        eli5: 'Calling the pizza shop and they don\'t hang up until your pizza is ready.',
        technical: 'Client sends request, server waits for event, responds, client immediately reconnects. More efficient than short polling for frequent updates. Server holds connections (resource intensive). Used by: Facebook chat (legacy).',
      },
      {
        title: 'WebSockets',
        content: 'Persistent bidirectional connection between client and server.',
        eli5: 'Like a phone call that stays connected - either side can talk anytime.',
        technical: 'Full-duplex communication over single TCP connection. Server can push to client. Low latency, efficient for high-frequency updates. Stateful: need connection management, reconnection logic. Use for: chat, gaming, live dashboards.',
      },
      {
        title: 'Server-Sent Events (SSE)',
        content: 'Server pushes updates to client over HTTP. One-way: server to client only.',
        eli5: 'Like a radio broadcast - the station sends, you listen.',
        technical: 'Uses standard HTTP. Automatic reconnection built into browser API. Simpler than WebSockets for server-to-client only updates. Limited browser support for POST bodies. Use for: notifications, live feeds, stock tickers.',
      },
    ],
    terminology: [
      { term: 'Full-Duplex', definition: 'Both sides can send data simultaneously' },
      { term: 'Half-Duplex', definition: 'Only one side can send at a time' },
      { term: 'Heartbeat', definition: 'Periodic message to keep connection alive' },
      { term: 'Reconnection', definition: 'Re-establishing connection after disconnect' },
    ],
    commonMistakes: [
      'Using polling when WebSockets would be more efficient',
      'Not implementing reconnection logic for WebSockets',
      'Long polling without connection timeouts (resource exhaustion)',
      'WebSockets for simple request-response (over-engineering)',
      'Not considering mobile networks (frequent disconnects)',
    ],
    relatedTopics: ['load-balancing', 'rate-limiting', 'event-driven'],
  },
